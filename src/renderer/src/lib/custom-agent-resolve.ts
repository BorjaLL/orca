...
/** Collapse the default-agent preference to a built-in id for the many flows
 *  that predate custom profiles (source-control AI, onboarding, fix-checks,
 *  floating terminal, etc.). A custom default resolves to its `baseAgent` so
 *  those flows still launch the right underlying CLI; a deleted profile, blank,
 *  or auto preference passes through unchanged. */
export function defaultBuiltinTuiAgent(
  settings: GlobalSettings | null | undefined
): TuiAgent | 'blank' | null {
  const pref = settings?.defaultTuiAgent
  if (pref && typeof pref === 'object') {
    return pref.kind === 'custom'
      ? (findCustomAgentProfile(settings, pref.id)?.baseAgent ?? null)
      : null
  }
  return pref ?? null
}
