/* eslint-disable max-lines -- Why: mirrors CommitMessageAiPane so each PR
   generation setting (toggle, agent dropdown, model dropdown, thinking effort,
   custom command, custom prompt) stays a single SearchableSetting block; the
   ~6 conditional render branches read clearest kept together. */
import { useEffect, useMemo, useRef, useState } from 'react'
import { RefreshCw, Terminal } from 'lucide-react'
import type { GlobalSettings, PullRequestAiSettings, TuiAgent } from '../../../../shared/types'
import {
  CUSTOM_AGENT_ID,
...
const EMPTY_SETTINGS: PullRequestAiSettings = {
  enabled: false,
  agentId: null,
...
const COMING_SOON_PULL_REQUEST_AGENTS: readonly { id: TuiAgent; label: string }[] = [
  { id: 'gemini', label: 'Gemini' }
]

// Why: PR settings fall back to the commit-message settings until the user
// edits this pane, matching the resolver so the displayed state reflects what
// generation would actually use.
function readSettings(settings: GlobalSettings): PullRequestAiSettings {
  return settings.pullRequestAi ?? settings.commitMessageAi ?? EMPTY_SETTINGS
}
...
function selectModelForHost(
  ...
  agentId: TuiAgent,
  modelId: string
): Pick<PullRequestAiSettings, 'selectedModelByAgent' | 'selectedModelByAgentByHost'> {
...
export function PullRequestAiPane({
  settings,
  updateSettings,
  onCustomPromptDirtyChange,
  customPromptDiscardSignal
}: PullRequestAiPaneProps): React.JSX.Element {
  ...
  const unsupportedDefaultAgentLabel = unsupportedDefaultAgent
    ...
    : null
  const unsupportedSelectedAgentIsComingSoon = COMING_SOON_PULL_REQUEST_AGENTS.some(
    (agent) => agent.id === unsupportedSelectedAgent
  )
  const unsupportedSelectedAgentLabel = unsupportedSelectedAgent
    ? (COMING_SOON_PULL_REQUEST_AGENTS.find((a) => a.id === unsupportedSelectedAgent)?.label ??
  ...
  const writeConfig = (patch: Partial<PullRequestAiSettings>): void => {
    updateSettings({ pullRequestAi: { ...config, ...patch } })
  }
  ...
  const refreshModels = async (agentId: TuiAgent): Promise<void> => {
    ...
    try {
      ...
      updateSettings({
        pullRequestAi: mergeDiscoveredModelsIntoCommitMessageConfig(
          latestConfig,
          agentId,
          result.models,
  ...
  const onSaveCustomPrompt = async (): Promise<void> => {
    ...
    try {
      await updateSettings({ pullRequestAi: { ...config, customPrompt: customPromptDraft } })
    } finally {
  ...
  if (
    matchesSettingsSearch(searchQuery, {
      ...
      keywords: ['ai', 'pull', 'request', 'pr', 'generate', 'agent', 'enabled']
    })
  ) {
    sections.push(
      <SearchableSetting
      ...
      >
        <div className="space-y-0.5">
          <Label>Enable AI pull request details</Label>
          <p className="text-xs text-muted-foreground">
            Adds a Generate button to the Create Pull Request dialog that drafts a title and
            description from your branch changes. Runs the agent CLI locally (or on the SSH host when
            working remotely) and waits for the response.
          </p>
        </div>
        <button
          role="switch"
    ...
    sections.push(
      ...
      >
        <div className="space-y-0.5">
          <Label>Agent</Label>
          <p className="text-xs text-muted-foreground">
            Which agent drafts your pull request details. Orca invokes its CLI in the background, so
        ...
        <div className="flex flex-col items-end gap-1">
          <Select value={activeAgentSelectValue} onValueChange={onAgentChange}>
            ...
            <SelectContent>
              ...
              {COMING_SOON_PULL_REQUEST_AGENTS.filter(
          ...
          {unsupportedDefaultAgentLabel ? (
            <p className="max-w-[260px] text-right text-[11px] text-muted-foreground">
              Your default agent is {unsupportedDefaultAgentLabel}, which does not support pull
              request generation yet. Choose a supported agent or Custom.
            </p>
          ) : null}
          {unsupportedSelectedAgentLabel ? (
            <p className="max-w-[260px] text-right text-[11px] text-muted-foreground">
              {unsupportedSelectedAgentIsComingSoon
                ? `${unsupportedSelectedAgentLabel} pull request generation is coming soon.`
                : `${unsupportedSelectedAgentLabel} does not support pull request generation yet.`}{' '}
  ...
  if (
    ...
    sections.push(
      ...
      >
        <div className="space-y-0.5">
          <Label htmlFor="pull-request-ai-custom-command">Custom command</Label>
          <p className="text-xs text-muted-foreground">
            Use{' '}
        ...
        <input
          id="pull-request-ai-custom-command"
          type="text"
          spellCheck={false}
          autoCorrect="off"
          autoCapitalize="off"
    ...
    sections.push(
      ...
      >
        <div className="space-y-0.5">
          ...
          <p className="text-xs text-muted-foreground">
            Higher effort produces more careful details but takes longer and costs more tokens.
          </p>
        </div>
  ...
  if (
    ...
    sections.push(
      <SearchableSetting
        key="custom-prompt"
        title="Custom prompt"
        description="Optional instructions appended to the base prompt (e.g. required PR description sections)."
        keywords={['prompt', 'description', 'template', 'style']}
        forceVisible={isCustomPromptDirty}
        className="space-y-2 py-2"
      >
        <div className="space-y-0.5">
          <Label htmlFor="pull-request-ai-custom-prompt">Custom prompt</Label>
          <p className="text-xs text-muted-foreground">
            Appended verbatim to the base prompt. Use it to enforce a description template, required
            sections, testing-notes conventions, or any other style your team prefers.
          </p>
        </div>
        <textarea
          id="pull-request-ai-custom-prompt"
          rows={4}
          value={customPromptDraft}
          onChange={(e) => setCustomPromptDraft(e.target.value)}
          placeholder="Start the body with a Summary section, then a Test plan checklist. Reference the ticket key when present."
  ...
  // AI Commit Messages rows above.
  return (
    <div className="space-y-4 border-t border-border/40 pt-4">
      <div className="space-y-0.5">
        <h3 className="text-sm font-semibold">AI Pull Requests</h3>
        <p className="text-xs text-muted-foreground">
          Generate pull request titles and descriptions from your branch changes using a local agent
          CLI. Falls back to your AI Commit Messages settings until you configure this pane.
        </p>
      </div>
      {sections}
    </div>
