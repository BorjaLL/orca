...
export const TERMINAL_COMMAND_SPECS: CommandSpec[] = [
  ...
  },
  {
    ...
    notes: [
      'Without --enter the text is pasted into the composer and never submitted; the call still succeeds. Pass --enter whenever the text is an instruction you expect to be acted on.'
    ]
  },
  {
    path: ['terminal', 'wait'],
  ...
  },
  {
    ...
    usage:
      'orca terminal create [--worktree <selector>] [--title <name>] [--command <text>] [--focus] [--verify-command] [--recover-command] [--verify-timeout-ms <ms>] [--json]',
    allowedFlags: [
      ...GLOBAL_FLAGS,
      'worktree',
      'command',
    ...
    notes: [
      ...
      'create returns as soon as the tab exists, before --command is delivered. Pass --verify-command to wait for the command to appear in the terminal and exit non-zero if it never does; --recover-command additionally resends it once on a drop.'
    ],
    examples: [
      ...
      'orca terminal create --worktree active --command "codex" --recover-command --json'
    ]
  },
  {
