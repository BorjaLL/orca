...
3. **Pressure builds on the product-telemetry lane.** Every bug report that could have been diagnosed from trace data becomes an argument to add "just this one raw field" to the PostHog schema. local-first tracing design's architecture explicitly keeps that pressure off by putting errors in a different place. We should too.
...
- **local-first tracing reference**, **Ghostty**, **VS Code** separate the two lanes at the *architecture* level — error data lives in a different code path with different consent gates and different transport than product telemetry.
...
Scope is the same as local-first tracing design's: completed spans from the main process, with their attributes, events, and exit outcome.
...
Modeled on local-first tracing design's `docs/observability.md` §"Run The Server In Instrumented Mode", adapted for the Electron main process.
...
- File rotation: 10 MB × 10 files (matches local-first tracing design's defaults). Oldest deleted on rotation.
...
Paired with local-first tracing design's documented `docker run grafana/otel-lgtm` command, a user can stand up a full trace UI in ~30 seconds. The data never leaves their machine (or their infra if they run LGTM elsewhere).
...
src/main/observability/   # NEW — the error-tracking lane
  ...
  tracer.ts              # span recorder modeled on local-first tracing design's TraceSink
...
local-first tracing design's list in `docs/observability.md` §"How To Think About Adding Tracing To Future Code" maps cleanly onto Orca:
...
- **Disk on the user's machine:** 10 files × 10 MB = 100 MB worst case, FIFO rotated. Matches local-first tracing design's default and has caused no reported complaints there.
- **Process overhead:** Effect's local-file tracer batches every 200 ms (local-first tracing design's default). Main-process cost is dominated by the serialization, not the I/O; negligible at our span volume.
- **Engineering:** the local-file tracer + rotation is ~300 LOC following local-first tracing design's implementation (`apps/server/src/observability/LocalFileTracer.ts`, `TraceSink.ts`). OTLP export is already a first-party Effect module (`OtlpTracer.make`), no custom transport. Diagnostic bundle preview + upload is ~200 LOC renderer + ~100 LOC main. Total ~600 LOC v1.
...
## What to copy from local-first tracing reference verbatim

Files from `/Users/thebr/source/repos/public/reference-observability` that are direct implementation references:
...
What **not** to copy: the `Identify.ts` file that hashes provider account IDs into a `distinct_id`. That's local-first tracing design's product-telemetry identity, already rejected in Orca's telemetry plan. The error-tracking lane doesn't need an identifier at all.
...
**local-first tracing reference** (`apps/server/src/observability/`). The design we're porting. Errors live in Effect span exit status (`TraceRecord.ts:35-46`) — each completed span carries `exit: { _tag: "Success" | "Interrupted" | "Failure", cause: string }`, where `cause` is `Cause.pretty(exit.cause)` (the formatted stack). NDJSON is the persistent source of truth; OTLP is an opt-in mirror for operators running Grafana LGTM. PostHog schema carries zero error events — this is the *only* peer in the survey whose product telemetry has no error signal at all, because the error signal is deliberately elsewhere.

Why this works: the error representation (span exit) is richer than a stack trace alone — it ties the failure to its enclosing request, the sibling spans, and the attributes at each level. This architecture can afford to be strict on product-telemetry UGC precisely because span data picks up the slack.
...
|---|---|---|---|---|---|
| **local-first tracing reference** | A — structured tracing, local-first | Span exit status + `Cause.pretty` | Local NDJSON (opt-in OTLP) | Env vars only | `apps/server/src/observability/TraceRecord.ts:35-46` |
...
1. **Strategy A (our port) is the rarest choice in the set.** Only local-first tracing reference ships it. That's a feature, not a bug — it's what lets us have both rich diagnostic data *and* an enum-only product-telemetry schema without either lane compromising the other.
...
- local-first tracing reference observability implementation: `/Users/thebr/source/repos/public/reference-observability/apps/server/src/observability/` (verified 2026-05-01). Span exit shape at `TraceRecord.ts:35-46`.
- local-first tracing reference user-facing docs: `/Users/thebr/source/repos/public/reference-observability/docs/observability.md`.
- local-first tracing reference absence of error events in product schema: `grep -n "analytics.record" /Users/thebr/source/repos/public/reference-observability/apps/server/src/**/*.ts` → 9 call sites, none carrying error strings. Transport at `apps/server/src/telemetry/Layers/AnalyticsService.ts`.
...
- Grafana LGTM single-container stack: `grafana/otel-lgtm` Docker image, referenced in local-first tracing design's `docs/observability.md` §"Run With A Local LGTM Stack".
