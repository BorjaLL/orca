...
- `terminal send` without `--enter` pastes the text into the composer and never submits it, and still reports success. Always pass `--enter` when the text is an instruction you expect the agent to act on; the CLI warns on stderr when it looks like you forgot.
- `terminal create` returns as soon as the tab exists, before `--command` is delivered, so a returned handle does not prove the agent started. Pass `--recover-command` (or `--verify-command` to only check) when the command matters: the CLI then waits for the command to appear in the terminal, resends it once if it was dropped, and exits non-zero if it never lands.
- `terminal create` returning `handlePending: true` means the tab exists; never create another for the same request. Keep the handle: it binds once the terminal registers. Commands against it fail with `terminal_handle_pending` until then, so retry the same handle rather than treating that as a failed create.
